# Parses Dictionary Data to Output All Family Data
...
def displayFamilyData(individualData, familyData):
    ...
    famDataTable.field_names = ["ID", "Married", "Divorced",
    ...
    for id in idListSorted:
        ...
        if (marriedDate != 'N/A'):
            ...
            married = marriedDateSplit[2] + '-' + \
                monthToNumDict[marriedDateSplit[1]] + \
                '-' + marriedDateSplit[0].zfill(2)
...
def main(fileName, GEDCOM_dict):
    ...
    if (fileName.endswith('_dict.json')):
        ...
        familyDataTable = displayFamilyData(
            GEDCOM_dict['individualData'], GEDCOM_dict['familyData'])
        ...
        # Overwrite GEDCOM Created in 1_storeDataAsDict.py with Data Matching Tables
    ...
    # Case when the Program is run from 0_main.py
        ...
        if (len(GEDCOM_dict) != 0):
            ...
            # Overwrite GEDCOM Created in 1_storeDataAsDict.py
            with open(fileName[:-4] + '_dict.json', 'w', encoding='utf-8') as outfile:
                json.dump(GEDCOM_dict, outfile, ensure_ascii=False, indent=2)
