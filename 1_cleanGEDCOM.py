#  Creates a New Txt ( {filename}_parse.txt ) with line by line Validation
...
def validate(line):
    validTagLevelPairs = {
        'INDI': 0, 'FAM': 0, 'HEAD': 0, 'TRLR': 0, 'NOTE': 0,
        'NAME': 1, 'SEX': 1, 'BIRT': 1, 'DEAT': 1, 'FAMC': 1,
        'FAMS': 1, 'MARR': 1, 'HUSB': 1, 'WIFE': 1, 'CHIL': 1, 'DIV': 1,
        'DATE': 2
    }
    ...
    # Special Case because we need to parse INDI/FAM tags differently
    if (level == 0):
...
def main(fileName):
    ...
    clean = open(fileName[:-4] + 'cleaned.ged', 'w+')
    ...
    for line in inputFile:
        ...
        if (str(valid) == "Y"):
            clean.write(lineValidation + " | " + str(lineCounter) + "\n")
